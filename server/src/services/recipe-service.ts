...
export class RecipeService {
  ...
  async createRecipe(input: CreateRecipeInput): Promise<RecipeWithDetails> {
    ...
    const [recipe] = await db.insert(schema.recipes).values(recipeData).returning();
    ...
    const steps = await db.insert(schema.recipeSteps).values(stepData).returning();
    ...
    if (input.tags && input.tags.length > 0) {
      ...
      await db.insert(schema.recipeTags).values(tagData);
  ...
  async updateRecipe(id: string, input: UpdateRecipeInput): Promise<RecipeWithDetails | null> {
    ...
    await db.delete(schema.recipeTags).where(eq(schema.recipeTags.recipeId, id));
